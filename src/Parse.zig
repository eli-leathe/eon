...
test "parse empty source" {
    try expectNodeTags("", &.{.root});
}

test "parse declarations and primary expressions" {
    const source =
        \\name = value
        \\number = 42
        \\string = "hello"
    ...
    defer arena.deinit();
    var ast = try parse(std.testing.allocator, arena.allocator(), source);
    defer ast.deinit(std.testing.allocator) catch unreachable;

    try std.testing.expectEqual(@as(usize, 0), ast.errors.len);
    try expectAstNodeTags(&ast, &.{
        .root,
        .identifier,
        .declaration,
        .number_literal,
    ...
    const declarations = ast.nodes.items(.data)[0].root;
    try std.testing.expectEqualSlices(Ast.Node.Index, &.{
        @fromBackingInt(@intCast(2)),
        @fromBackingInt(@intCast(4)),
        @fromBackingInt(@intCast(6)),
        @fromBackingInt(@intCast(8)),
    }, declarations);
}

test "parse operator precedence, groups, and suffix expressions" {
    try expectNodeTags(
        "value = -a.b.c * (d + e) / f - g == h and i or j",
        &.{
            .root,
            .identifier,
            .field_access,
...
test "parse binary operators are left associative" {
    try expectNodeTags(
        "value = a - b - c / d / e",
        &.{
            .root,
...
test "parse errors" {
    const Case = struct {
        source: [:0]const u8,
        token_tag: Token.Tag,
        error_data: Ast.Error.Data,
    };
    const cases: []const Case = &.{
        .{ .source = "input", .token_tag = .eof, .error_data = .{ .expected_token = .equal } },
        .{ .source = "input =", .token_tag = .eof, .error_data = .expected_expr },
        .{ .source = "input = -", .token_tag = .eof, .error_data = .expected_prefix_expr },
        .{ .source = "input = (value", .token_tag = .eof, .error_data = .{ .expected_token = .r_paren } },
        .{ .source = "input = value +", .token_tag = .eof, .error_data = .expected_expr },
        .{ .source = "input = a == b == c", .token_tag = .equal_equal, .error_data = .chained_comparison_operators },
        .{ .source = "input = value.", .token_tag = .eof, .error_data = .expected_suffix_op },
        .{ .source = "42", .token_tag = .number_literal, .error_data = .{ .expected_token = .eof } },
    };

    for (cases) |case| {
        var arena = std.heap.ArenaAllocator.init(std.testing.allocator);
        defer arena.deinit();
        var ast = try parse(std.testing.allocator, arena.allocator(), case.source);
        defer ast.deinit(std.testing.allocator) catch unreachable;

        try std.testing.expectEqual(@as(usize, 1), ast.errors.len);
        const parse_error = ast.errors[0];
        try std.testing.expectEqual(case.token_tag, ast.tokens.items(.tag)[parse_error.token]);
        try std.testing.expectEqualDeep(case.error_data, parse_error.data);
    }
}

fn expectNodeTags(source: [:0]const u8, expected: []const Ast.Node.Tag) !void {
    var arena = std.heap.ArenaAllocator.init(std.testing.allocator);
    defer arena.deinit();
    var ast = try parse(std.testing.allocator, arena.allocator(), source);
    defer ast.deinit(std.testing.allocator) catch unreachable;
    ...
    try std.testing.expectEqual(@as(usize, 0), ast.errors.len);
    try expectAstNodeTags(&ast, expected);
}

fn expectAstNodeTags(ast: *const Ast, expected: []const Ast.Node.Tag) !void {
    const actual = ast.nodes.items(.data);
    try std.testing.expectEqual(expected.len, actual.len);
    for (expected, actual) |expected_tag, node_data| {
        try std.testing.expectEqual(expected_tag, std.meta.activeTag(node_data));
    }
}
